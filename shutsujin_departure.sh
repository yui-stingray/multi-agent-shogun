...
if ! tmux has-session -t shogun 2>/dev/null; then
    if ! tmux new-session -d -s shogun -n main; then
        echo ""
        echo "  ╔════════════════════════════════════════════════════════════╗"
        echo "  ║  [ERROR] tmux セッション 'shogun' の作成に失敗           ║"
        echo "  ║  tmux ls で既存セッションを確認してください               ║"
...
PANE_BASE=$(tmux show-options -gwv pane-base-index 2>/dev/null || echo 0)
